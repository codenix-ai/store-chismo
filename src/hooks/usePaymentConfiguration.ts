...
export const usePaymentConfigurations = () => {
  ...
  const [createConfiguration, { loading: creating, error: createError }] = useMutation(CREATE_PAYMENT_CONFIGURATION, {
    refetchQueries: [{ query: GET_PAYMENT_CONFIGURATIONS }],
    errorPolicy: 'all',
  });

  const [updateConfiguration, { loading: updating, error: updateError }] = useMutation(UPDATE_PAYMENT_CONFIGURATION, {
    refetchQueries: [{ query: GET_PAYMENT_CONFIGURATIONS }],
    errorPolicy: 'all',
  });
...
export const useStorePaymentConfiguration = () => {
  const { configurations, loading, error, createConfiguration, updateConfiguration } = usePaymentConfigurations();
  ...
  const setupMercadoPagoConfiguration = async (config: { publicKey: string; apiKey: string; testMode: boolean }) => {
    const input: CreatePaymentConfigurationInput = {
  ...
  const setupEpaycoConfiguration = async (config: { publicKey: string; apiKey: string; testMode: boolean }) => {
    const input: CreatePaymentConfigurationInput = {
      epaycoEnabled: true,
